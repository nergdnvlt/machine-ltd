...
class UserEndpointTest(TestCase):
    ...
    def test_update_put_user_endpoint(self):
        ...
        response = self.client.put(f'/api/v1/users/{user_id}', {'user': {'username': 'Thor', 'phone_number': '7192710056'}}, format='json')
        end_user = response.json()

        self.assertEqual(end_user['phone_number'], '7192710056')


    def test_sad_path_put_user_endpoint(self):
        response = self.client.put('/api/v1/users/10001', {'user': {'username': 'Thor', 'phone_number': '7195558888'}}, format='json')
    ...
    def test_patch_user_endpoint(self):
        ...
        response = self.client.patch(f'/api/v1/users/{user_id}', {'user': {'username': 'Thor', 'phone_number': '7192710056'}}, format='json')
        end_user = response.json()

        self.assertEqual(end_user['phone_number'], '7192710056')

    def test_sad_path_patch_user_endpoint(self):
        response = self.client.patch('/api/v1/users/10001', {'user': {'username': 'Thor', 'phone_number': '7192291210'}}, format='json')
